document.addEventListener('DOMContentLoaded', () => {
    ...
    try {
      ...
      if (Array.isArray(data.warnings)) {
        data.warnings.forEach(message => {
          if (message) {
            appendConsoleLine(`NOTICE: ${message}`, 'info');
          }
        });
      }
      if (!response.ok) {
      ...
      if (!results.length) {
        ...
        let prefix = 'Showing';
        if (data && data.cached) {
          prefix = 'Showing cached';
        } else if (data && data.source === 'web') {
          prefix = 'Showing quick';
        } else if (data && data.source === 'yt-dlp') {
          prefix = 'Showing detailed';
        }
  ...
  function ensurePollerMap() {
    if (!state.pollers || typeof state.pollers.set !== 'function') {
